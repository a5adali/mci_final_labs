...
float sensitivity = 0.00875f; // 8.75 mdps/LSB
...
int16_t read_register_16(uint8_t low_addr)
{
    uint8_t tx[2];
    uint8_t rx[2];
    tx[0] = READ_CMD | low_addr | 0x40;  // bit6=1 for auto-increment
    rx[0] = 0;
    rx[1] = 0;
...
/**
  ...
  *         where the assert_param error has occu/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Main program body
