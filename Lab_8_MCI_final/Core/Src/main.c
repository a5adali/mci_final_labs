...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* If ISR produced a new sample, send it from main context */
    if (temp_ready) {
      temp_ready = 0; // clear flag
      int n = snprintf(uart_buf, sizeof(uart_buf), "%d\r\n", (int)latest_temp);
      if (n > 0) {
        HAL_UART_Transmit(&huart1, (uint8_t*)uart_buf, n, HAL_MAX_DELAY);
      }
    }

    HAL_Delay(100); // request ~10Hz read (100ms); adjust as needed
  /* USER CODE END 3 */
}
}
void gyro_init(void)
